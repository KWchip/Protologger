...
print ('Code is running in; ', code_path)
...
bashCommand = 'blastn -query ' + File_16S + ' -db ' + code_path + 'bin/16S-SILVA/LTP-DB/LTP -evalue 0.0000000000000000000000001 -perc_identity 60 -outfmt 6 -out ' + dir_path + project_name +'/LTP-matches.m8' 
#print bashCommand
...
for line in open(dir_path +project_name+'/LTP-matches.m8'):
    timber = line.replace('\n','').split('\t')
    hit = timber[1].split('\|')[2]
...
num = 0
for line in open(code_path + 'bin/DSMZ-valid-list/DSMZ-August-2019.tab','r'):
    num +=1
    if num >1:
        timber = line.replace('\n','').split('\t')
        if len(timber) > 1:
...
outputting = open(dir_path +project_name+'/Matching-16S-sequences.fasta','w')
...
outputting_overview.write('Protologger v1.1; Run date: ' + str(today) + '\n')
...
match_16S_length = 0
for read in HTSeq.FastaReader(dir_path +project_name+'/Matching-16S-sequences.fasta'):
    if read.name.split('--')[2] == top_16S_match:
        match_16S_length = len(read.seq.decode("utf-8"))
        
user_16S_length = 0
for read in HTSeq.FastaReader(File_16S):
    user_16S_length = len(read.seq.decode("utf-8"))
...
if args.command == "16s":
    ...
    #Run GTDB-TK
    bashCommand = 'checkm lineage_wf --reduced_tree -x fna ' + dir_path +project_name+' '+ dir_path  +project_name+'/CheckM_results' 
    #print (bashCommand)
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    ...
    bashCommand = 'rm -r ' + dir_path +project_name+'/CheckM_results' 
    #print bashCommand
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()
...
for prot1 in HTSeq.FastaReader(File_16S):
    #print prot1.name
    protdat = {}
    for prot2 in HTSeq.FastaReader(dir_path +project_name+'/Matching-16S-sequences.fasta'):
        #print prot2.name
        if prot1.name == prot2.name:
            continue
        else:
            outfile = dir_path + project_name + '/' + prot1.name + '--' + prot2.name 
            outputting = open(outfile + '.fasta','w')
            outputting.write('>' + prot1.name + '\n')
            outputting.write(prot1.seq.decode("utf-8") + '\n')
            outputting.write('>' + prot2.name + '\n')
            outputting.write(prot2.seq.decode("utf-8") + '\n')
            outputting.close()
            ...
            for read in HTSeq.FastaReader(outfile + '.afa'):
                if read.name == prot1.name:
                    seq1 = read.seq.decode("utf-8")
                elif read.name == prot2.name:
                    seq2 = read.seq.decode("utf-8")
            ##print seq1
            ##print seq2
...
outputting = open(dir_path +project_name+'/16S-identity-values.tab','w')

outputting.write('#Matching species' + '\t' + 'Similarity (%)' + '\n')

matches = list(datas.values())[0]
...
if most_similar < 70.0:
    output_rev = open(dir_path +project_name+'/Reverse_complemenet_16S.fasta','w')
    for seq in HTSeq.FastaReader(File_16S):
        output_rev.write('>' + seq.name + '\n')
        output_rev.write(reverse_complement(seq.seq.decode("utf-8")) + '\n')
    output_rev.close()

    rev_16S = dir_path +project_name+'/Reverse_complemenet_16S.fasta'
...
outputting = open(dir_path +project_name+'/Combined.fasta','w')
for seq in HTSeq.FastaReader(File_16S):
    outputting.write('>' + seq.name + '\n')
    outputting.write(seq.seq.decode("utf-8") + '\n')
for seq in HTSeq.FastaReader(dir_path +project_name+'/Matching-16S-sequences.fasta'):
    outputting.write('>' + seq.name + '\n')
    outputting.write(seq.seq.decode("utf-8") + '\n')
outputting.close()
...
outputting = open(dir_path +project_name+'/16S-tree.nwk','w')

bashCommand = 'fasttree -quiet -gtr -nt ' + dir_path +project_name+'/Combined.afa'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

outputting.write(output.decode("utf-8"))
outputting.close()

bashCommand = 'rm ' + dir_path +project_name+'/Combined.fasta' 
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm ' + dir_path +project_name+'/Combined.afa' 
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
ln = 0
for line in open(code_path + 'bin/GTDB-TK/bac120_metadata.tsv','r'):
    ln +=1
    if ln >1:
        timber = line.replace('\x00','').replace('\n','').split('\t')
        if len(timber) > 1:
            genome = timber[0]
            #print (timber)
...
ln = 0
for line in open(code_path + 'bin/GTDB-TK/ar122_metadata.tsv','r'):
    ln +=1
    if ln >1:
        timber = line.replace('\x00','').replace('\n','').split('\t')
        if len(timber) > 1:
...
if args.quick == True:
    print ('Ignoring GTDB-Tk as quick flag is active.')
else:
    print ('Starting GTDB-TK')


    bashCommand = 'gtdbtk classify_wf --genome_dir ' + dir_path +project_name+' --out_dir ' + dir_path +project_name+'/GTDB-TK-Results --cpus 20' 
    #print (bashCommand)
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)                          #################
    output, error = process.communicate()                          #################
    #print (output)
    ...
    if os.path.isfile(dir_path +project_name+'/GTDB-TK-Results/classify/gtdbtk.bac120.summary.tsv') == True:
        for line in open(dir_path +project_name+'/GTDB-TK-Results/classify/gtdbtk.bac120.summary.tsv','r'):                          #################
            ln +=1                          #################
            if ln > 1:                          #################
                timber = line.replace('\n','').split('\t')                          #################
                ##print timber
                #print 'Genome based assignment via GTDBK-TK placed the genome as; ', timber[1]
                outputting_overview.write('Genome based assignment via GTDB-TK placed the genome as; ' + timber[1] +'\n')                          #################
                if timber[2] == 'N/A':                          #################
                ...
                if timber[14] != 'N/A':                          #################
                    ...
                    for i in timber[14].split(';'):                          #################
                        ...
                        #print gID
                        for k,v in Representative_genomes.items():                          #################
                            if v == gID:                          #################
        ...
        if os.path.isfile(dir_path +project_name+'/GTDB-TK-Results/classify/gtdbtk.ar122.summary.tsv') == True:
            for line in open(dir_path +project_name+'/GTDB-TK-Results/classify/gtdbtk.ar122.summary.tsv','r'):                          #################
                ln +=1                          #################
                if ln > 1:                          #################
                    timber = line.replace('\n','').split('\t')                          #################
                    ##print timber
...
for line in open(dir_path +project_name+'/Matching-16S-sequences.fasta','r'):
    if line.startswith('>'):
...
for genome in Genomes_for_comparison:
    ...
    for k,v in Representative_genomes.items():
        if genome == v:
            taxa = k
            print (genome , taxa)
            bashCommand = 'bash tmp.tmp'
            outputting = open('tmp.tmp','w')
            outputting.write('cp ${GTDBTK_DATA_PATH}fastani/database/' + genome.replace('GB_','').replace('RS_','').split('_')[0] + '/' + genome.replace('GB_','').replace('RS_','').split('_')[1][0:3] + '/' + genome.replace('GB_','').replace('RS_','').split('_')[1][3:6] + '/' + genome.replace('GB_','').replace('RS_','').split('_')[1][6:9] + '/' + genome.replace('GB_','').replace('RS_','') + '_genomic.fna.gz ' + dir_path +project_name+'/Genome_analysis/' + taxa + '--' + genome + '_genomic.fna.gz') 
            outputting.close()
            print (bashCommand)
            process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
            output, error = process.communicate()
...
for i in glob.glob(dir_path +project_name+'/Genome_analysis/*.gz'):    
...
for read in HTSeq.FastaReader(genome_file):
    for i in read.seq.decode("utf-8"):
        tot +=1
...
outputting_gc_content = open(dir_path +project_name+'/GC_of_closest_relatives.tab','w')
outputting_gc_content.write('#Species\tGC%\n')
for genome in glob.glob(dir_path +project_name+'/Genome_analysis/*.fna'):
    #print genome
    gc = 0
    tot = 0
...
for line in open(dir_path + project_name + '/ANI_values.tab','r'):
    timber = line.replace('\n','').split('\t')
...
bashCommand = 'rm ' + dir_path +project_name+'/comparison_list.txt '
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
...
for cfile in glob.glob(dir_path +project_name+'/Genome_analysis/*.fna'):                    #########
    bashCommand = 'mkdir -p ' + dir_path +project_name+'/Genome_analysis/'+cfile.split('/')[-1:][0].replace('.fna','')                    #########
    #print bashCommand
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)                    #########
    output, error = process.communicate()                    #########
    bashCommand = 'prodigal -i ' + cfile + ' -c -m -a ' + dir_path +project_name+'/Genome_analysis/'+cfile.split('/')[-1:][0].replace('.fna','') + '/' + cfile.split('/')[-1:][0].replace('.fna','') + '.faa'                    #########
    #print bashCommand
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)                    #########
    output, error = process.communicate()                    #########
...
bashCommand = 'prokka --centre C --locustag L '+genome_file+' -o ' + dir_path +project_name+'/PROKKA-annotation --prefix Isolate'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
saved_lines = []
for line in open(dir_path +project_name+'/PROKKA-annotation/Isolate.log'):
    if 'Found' in line:
        if 'CRISPR' in line:
...
#bashCommand = 'prodigal -c -m -i ' + genome_file + ' -a ' + dir_path +project_name + '/PROKKA-annotation/Isolate.faa'
#print (bashCommand)
...
if is_quick == True:
    ...
    for genome in glob.glob(dir_path +project_name+'/Genome_analysis/*/*.faa'):
        bashCommand = 'cp  '+genome+' ' + dir_path +project_name+'/Genome_Tree/' 
        #print bashCommand
        process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
        output, error = process.communicate()

    bashCommand = 'cp  ' + dir_path +project_name+'/PROKKA-annotation/Isolate.faa ' + dir_path +project_name+'/Genome_Tree/' 
    #print bashCommand
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()

    bashCommand = 'phylophlan_write_default_configs.sh ' + dir_path +project_name
    print (bashCommand)
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)                      ################
    output, error = process.communicate()                      ################


    bashCommand = 'phylophlan -i ' + dir_path +project_name+'/Genome_Tree -d phylophlan --diversity medium  -f ' + dir_path + project_name + '/supermatrix_aa.cfg --nproc 20 --output_folder ' + dir_path + project_name
    print (bashCommand)
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)                      ################
    output, error = process.communicate()                      ################

    bashCommand = 'cp  ' + dir_path +project_name+'/Genome_Tree_phylophlan/RAxML_bestTree.Genome_Tree_refined.tre ' + dir_path +project_name+'/Genome_Tree.nwk' 
    #print bashCommand
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()
...
if is_quick == True:
    ...
    aimed_for = ''
    for genome in glob.glob(dir_path +project_name+'/PROKKA-annotation/*.faa'):
        genomes.append(genome)
        aimed_for = genome
    #for i in genomes:
        #print i
    ...
    #print ('At the POCP analysis part.')
    ...
    for FILE1 in genomes:
        ...
        for FILE2 in genomes:
            if aimed_for in [FILE1, FILE2]:
                bashCommand = 'blastp -evalue 0.00001 -qcov_hsp_perc 50.0 -subject ' + FILE2 + ' -query ' + FILE1 + ' -outfmt 6 > ' + dir_path + project_name + '/temp.m8'  
                print (bashCommand)
                notneeded = call(bashCommand, shell=True)                             ################
                #print notneeded
                total = 0
                ...
                matches = []
                for line in open(dir_path + project_name + '/temp.m8'):
                    timber = line.split('\t')
    ...
    bashCommand = 'rm ' + dir_path +project_name+'/temp.m8' 
    #print bashCommand
    process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()
    ...
    outputting_pocp = open(dir_path +project_name+'/POCP_results.tab','w')
    outputting_pocp.write('#Matching species\tPOCP (%)\n')
    assigned_genus = ''
    assigned_pocp = 0.0

    for genome1 in glob.glob(dir_path +project_name+'/PROKKA-annotation/*.faa'):
...
bashCommand = code_path +'bin/KEGG-analysis/kofam_scan-1.3.0/exec_annotation -f mapper -p ' + code_path + 'bin/KEGG-analysis/profiles -k ' + code_path + 'bin/KEGG-analysis/ko_list --cpu 20 -o ' + dir_path +project_name+'/KEGG_profile.KOs ' + dir_path +project_name+'/PROKKA-annotation/Isolate.faa'
print (bashCommand)
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
KOs = []
for line in open(dir_path +project_name+'/KEGG_profile.KOs','r'):
    timber = line.replace('\n','').split('\t')
    if len(timber) > 1:
        ko = timber[1]
        KOs.append(ko)
...
for line in open(code_path + 'bin/KEGG-analysis/brite.map','r'):
    timber = line.replace('\n','').split('\t')
...
if 'K00169' in KOs and 'K00170' in KOs and 'K00171' in KOs and 'K00172' in KOs:
    if 'K01007' in KOs:
        if 'K01595' in KOs:
            if 'K00024' in KOs:
                if 'K01677' in KOs and 'K01678' in KOs:
                    if 'K00239' in KOs and 'K00240' in KOs:
                        if 'K01902' in KOs and 'K01903' in KOs:
                            if 'K15038' in KOs:
                                if 'K14465' in KOs or 'K15017' in KOs:
                                    if 'K14467' in KOs:
                                        if 'K14534' in KOs and 'K15016' in KOs and 'K00626' in KOs:
                                            outputting_overview.write('The dicarboxylate-hydroxybutyrate cycle was predicted from the genome (EC:1.2.7.1, 2.7.9.2, 4.1.1.31, 1.1.1.37, 4.2.1.2, 1.3.5.1/1.3.5.4, 6.2.1.5, 1.2.1.76, 1.1.1.-, 6.2.1.40, 4.2.1.120, 4.2.1.17/1.1.1.35, 2.3.1.9)\n')
        ...
        if 'K16158' in KOs:
            if 'K16159' in KOs:
                if 'K16160' in KOs:
                    if 'K16161' in KOs:
                        if 'K16162' in KOs:
                            methanotroph.append('1.14.13.15')

if 'K14028' in KOs:
    methanotroph.append('1.1.2.7')
else:
    if 'K14029' in KOs:
        methanotroph.append('1.1.2.7')

if 'K23995' in KOs:
    methanotroph.append('1.1.2.10')


if len(methanotroph) == 3:
    outputting_overview.write('Oxidation of methane into formaldehyde was predicted from the genome (EC: ' + ', '.join(methanotroph)  + ')\n')
...
if 'K00370' in KOs and 'K00371' in KOs and 'K00374' in KOs:
    if 'K00362' in KOs and 'K00363' in KOs:
        outputting_overview.write('Dissimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.99.-/1.7.5.1, 1.7.1.15)\n')

if 'K00370' in KOs and 'K00371' in KOs and 'K00374' in KOs:
    if 'K03385' in KOs and 'K15876' in KOs:
        outputting_overview.write('Dissimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.99.-/1.7.5.1, 1.7.2.2)\n')

if 'K02567' in KOs and 'K02568' in KOs:
    if 'K00362' in KOs and 'K00363' in KOs:
        outputting_overview.write('Dissimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.9.6.1, 1.7.1.15)\n')

if 'K02567' in KOs and 'K02568' in KOs:
    if 'K03385' in KOs and 'K15876' in KOs:
        outputting_overview.write('Dissimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.9.6.1, 1.7.2.2)\n')
...
if 'K00367' in KOs:
    if 'K00366' in KOs:
        outputting_overview.write('Assimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.7.2, 1.7.7.1)\n')
    elif 'K17877' in KOs:
        outputting_overview.write('Assimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.7.2, 1.7.1.4)\n')
elif 'K10534' in KOs:
    if 'K00366' in KOs:
        outputting_overview.write('Assimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.1.1/1.7.1.2/1.7.1.3, 1.7.7.1)\n')
    elif 'K17877' in KOs:
        outputting_overview.write('Assimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.1.1/1.7.1.2/1.7.1.3, 1.7.1.4)\n')
elif 'K00372' in KOs and 'K00360' in KOs:
    if 'K00366' in KOs:
        outputting_overview.write('Assimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.99.-, 1.7.7.1)\n')
    elif 'K17877' in KOs:
        outputting_overview.write('Assimilatory nitrate reduction predicted via conversion of nitrate into ammonia (EC:1.7.99.-, 1.7.1.4)\n')
...
if 'K00370' in KOs:
    if 'K00371' in KOs:
        if 'K00374' in KOs:
            Denitrification.append('1.7.5.1/ 1.7.99.-')
else:
    if 'K02567' in KOs:
        if 'K02568' in KOs:
            Denitrification.append('1.9.6.1')

if 'K00368' in KOs:
    Denitrification.append('1.7.2.1')
else:
    if 'K15864' in KOs:
        Denitrification.append('1.7.2.1/1.7.99.1')


if 'K04561' in KOs:
    if 'K02305' in KOs:
        Denitrification.append('1.7.2.5')

if 'K00376' in KOs:
    Denitrification.append('1.7.2.4')

if len(Denitrification) == 4:
    outputting_overview.write('Denitrification was predicted via conversion of nitrate into nitrogen (EC: '+ ', '.join(Denitrification)+')\n')
...
bashCommand = 'mkdir -p ' + dir_path +project_name+'/CARD-annotation' 
print (bashCommand)
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'diamond blastp --query-cover 80.0 --subject-cover 80.0 --id 80.0 -q ' + dir_path +project_name+'/PROKKA-annotation/Isolate.faa -d ' + code_path + 'bin/CARD-annotation/CARDDB-DIAMOND -o ' + dir_path +project_name+'/CARD-annotation/'+project_name+'.CARD.m8' 
print (bashCommand)
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
Antibiotic_data = {}
ln = 0
for line in open(code_path + 'bin/CARD-annotation/aro_categories_index.tsv','r'):
    timber = line.replace('\n','').split('\t')
    ##print timber
    ln +=1
    if ln >1:
...
for line in open(dir_path +project_name+'/CARD-annotation/'+project_name+'.CARD.m8','r'):
    timber = line.replace('\n','').split('\t')
    query = timber[0]
    bitscore = float(timber[11])
    CARD = timber[1]
...
bashCommand = 'mkdir -p ' + dir_path +project_name+'/CAZy-annotation' 
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'diamond blastp --query-cover 40.0 --subject-cover 40.0 --min-score 100.0 -q ' + dir_path +project_name+'/PROKKA-annotation/Isolate.faa -d ' + code_path + 'bin/CAZy-annotation/CAZyDB-DIAMOND -o ' + dir_path +project_name+'/CAZy-annotation/'+project_name+'.CAZy.m8' 
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
for line in open(dir_path +project_name+'/CAZy-annotation/'+project_name+'.CAZy.m8','r'):
    timber = line.replace('\n','').split('\t')
    query = timber[0]
    hit = timber[1]
    bit = float(timber[11])
...
if len(GHs) > 0:
    ...
    outputting_overview.write('The following occurances of glycoside hydrolase families were identified within the genome; '+'\n')
...
if len(GTs) > 0:
    ...
    outputting_overview.write('The following occurances of glycoside transferase families were identified within the genome; '+'\n')
...
if len(PLs) > 0:
    ...
    outputting_overview.write('The following occurances of polysaccharise lyase families were identified within the genome; '+'\n')
...
if len(CEs) > 0:
    ...
    outputting_overview.write('The following occurances of carbohydrate esterase families were identified within the genome; '+'\n')
...
outputting = open(dir_path   + project_name +  '/ANI_values.txt','w')
for line in open(dir_path   + project_name +  '/ANI_values.tab'):
    outputting.write(line.replace(dir_path +project_name + '/', '').replace(dir_path +project_name + '/', '').replace('_genomic.fna','').replace('Genome_analysis/','').replace('.fna','').replace('dataset_','Input-'))
outputting.close()

bashCommand = 'rm '+dir_path  + project_name +  '/ANI_values.tab ' 
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  '+ dir_path + project_name + '/MAG_MASH.txt'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  '+ dir_path + project_name + '/MAG_MASH_v2.txt'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  '+ dir_path + project_name + '/genome.fasta'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  '+ dir_path + project_name + '/genome.fna'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  '+ dir_path + project_name + '/16S.fasta'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
...
#
#####
#bashCommand = 'rm -r '+ dir_path + project_name
#print bashCommand
#process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
#output, error = process.communicate()
bashCommand = 'rm  '+ dir_path + project_name + '/supermatrix_aa.cfg'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  '+ dir_path + project_name + '/supermatrix_nt.cfg'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  '+ dir_path + project_name + '/supertree_aa.cfg'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  '+ dir_path + project_name + '/supertree_nt.cfg'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()


bashCommand = 'rm  '+ dir_path + project_name + '/LTP-matches.m8'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  '+ dir_path + project_name + '/Matching-16S-sequences.fasta'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  '+ dir_path + project_name + '/Chimera_check.txt'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()

bashCommand = 'rm  -r '+ dir_path + project_name + '/CARD-annotation/'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  -r '+ dir_path + project_name + '/CAZy-annotation/'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  -r '+ dir_path + project_name + '/Genome_analysis/'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  -r '+ dir_path + project_name + '/Genome_Tree/'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  -r '+ dir_path + project_name + '/Genome_Tree_phylophlan/'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  -r '+ dir_path + project_name + '/IMNGS-analysis/'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()
bashCommand = 'rm  -r '+ dir_path + project_name + '/PROKKA-annotation/'
#print bashCommand
process = subprocess.Popen(bashCommand.split(), stdout=subprocess.PIPE)
output, error = process.communicate()


print ('Temporary files removed A-OK')




print ('Protologger has finished.')
